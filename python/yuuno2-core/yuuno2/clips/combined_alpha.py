...
class AlphaFrame(Frame):
    ...
    def native_format(self) -> RawFormat:
        ...
        return nf._replace(fields=[*nf.fields, "a"], alpha=True)
    ...
    async def can_render(self, format: RawFormat) -> bool:
        ...
        main_f = format._replace(fields=format.fields[:-1], alpha=False)
        alpha_f = format._replace(fields=["y"], family=ColorFamily.GREY, alpha=False)
